...
// nombre en inglés — inconsistencia interna de la librería. Se traduce acá
// en vez de depender de su i18n para ese único dato.
const SIGNOS_EN_ES = {
  Aries: 'Aries',
  Taurus: 'Tauro',
  Gemini: 'Géminis',
...
function longitudEclipticaSolYLuna({ year, month, date, hour, minute }) {
  ...
  const horoscope = construirHoroscopo({ year, month, date, hour, minute, lat: -33.4489, lon: -70.6693 });
  return {
...
function calcularSignoLunaLlena(year, month) {
  ...
  const horoscope = construirHoroscopo({ ...mejorInstante, lat: -33.4489, lon: -70.6693 });
  return {
    fecha: mejorInstante,
...
module.exports = { calcularCartaNatal, calcularSignoLunaLlena };
